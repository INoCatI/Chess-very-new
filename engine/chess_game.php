...
class ChessGame {
    ...
    public function createNewGame($human_color, $whiteID, $blackID) {
        ...
        $this->game_state->enable_castling_white_king = true; 
        $this->game_state->enable_castling_white_queen = true; 
        $this->game_state->enable_castling_black_king = true; 
        $this->game_state->enable_castling_black_queen = true; 
        $this->game_state->crossed_field = null;
        $this->game_state->non_action_semimove_counter = 0;
        $this->game_state->move_number = 1; 
    ...
    public function getClientJsonGameState() {
        ...
        return array(
            'with_ai' => $this->game_state->with_ai, // Добавлено новое значение в GameState (Указывает играем ли мы с компьютером)
    ...
    public function checkMate() { // Добавлен метод checkMate, меняет text_state в случае найденного мата/пата
        $ai = new ComputerAI($this->game_state);
        $move = $ai->getGenerateMove();
        if (!$move) {
            if ($this->isKingUnderAttack($this->game_state->current_player_color)) {
