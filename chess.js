...
let with_ai = true; // Добавили переменную 
let cells;
...
function initButtons() {
    ...
    button.addEventListener('click', (event) => createNewGame(event, 'offline')); // Добавили ещё "кнопку" для оффлайновой игры
...
function createNewGame(event, human_color) {
    ...
    with_ai = human_color != 'offline'; // Играем ли с AI (эта переменная спользуется в единственном месте в коде)
...
function move_click_processing(index, transform_to_figure) {
    ...
    is_our_move = false;
    if(with_ai) {
        setGameStatus('Ждите мой ответ'); // Вот это место (Где используется with_ai)
    }
...
function sendMoveToServer(cell_index_from, cell_index_to, transform_to_figure) {
    createRequest({
        ...
        callback: function(response) {
            setGameState(response);
            if(response.with_ai) { // Если играем с компьютером, то ожидаем ход компьютера
                if (response && response.is_human_move == false) {
                    sendWaitComputerMove();
                }
            }
            else { // Если нет, проверяем мат/пат
                checkMate();
            }
        }
    });
...
function checkMate() { // Запрос на проверку мата/пата
    createRequest({
        method: 'POST',
        url: 'check_mate.php',
        callback: function(response) {
